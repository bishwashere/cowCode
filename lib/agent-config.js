...
export function getAgentMessagingPolicy(agentId = DEFAULT_AGENT_ID) {
  ...
  const allow = Array.isArray(raw.allow)
    ? raw.allow.map((s) => normalizeAgentId(s)).filter(Boolean)
    : DEFAULT_AGENT_MESSAGING.allow;
  const maxDepth = Number.isFinite(raw.maxDepth) && raw.maxDepth > 0
    ? Math.floor(raw.maxDepth)
    : DEFAULT_AGENT_MESSAGING.maxDepth;
  const maxCallsPerTurn = Number.isFinite(raw.maxCallsPerTurn) && raw.maxCallsPerTurn > 0
    ? Math.floor(raw.maxCallsPerTurn)
