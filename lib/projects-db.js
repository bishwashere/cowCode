...
export function createProject({ name, description = '', url = '', setup_notes = '' }) {
  const db = getProjectsDb();
  const now = Date.now();
  const result = db.prepare(
    'INSERT INTO projects (name, url, description, setup_notes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(name, normalizeProjectUrl(url), description, String(setup_notes || ''), now, now);
  return getProject(result.lastInsertRowid);
}

export function updateProject(id, { name, description, url, setup_notes }) {
  const db = getProjectsDb();
  const now = Date.now();
  ...
  db.prepare('UPDATE projects SET name=?, url=?, description=?, setup_notes=?, updated_at=? WHERE id=?')
    .run(name, nextUrl, description, nextSetup, now, id);
