...
export function isLikelyContinuationReply(userText, historyMessages, implicitFeedback) {
  ...
  if (lastAssistant && hasNumberedOptions(lastAssistant) && wordCount(user) <= 4) return true;

  if (lastAssistant && asksForChoice(lastAssistant) && wordCount(user) <= 8) return true;
...
export function shouldSkipToolRetryProbe({ userText, historyMessages, intentPlan, implicitFeedback }) {
  if (isLikelyContinuationReply(userText, historyMessages, implicitFeedback)) return true;
  if (intentPlan?.mode === 'chat' && wordCount(userText) <= 4 && getLastAssistantMessage(historyMessages)) {
    return asksForChoice(getLastAssistantMessage(historyMessages))
      || hasNumberedOptions(getLastAssistantMessage(historyMessages));
  }
  return false;
}
