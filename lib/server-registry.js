 ...
 * @param {string} name - Alias (e.g. "prod", "staging")
 * @param {string} hostname - IP or DNS hostname
 * @param {{ user?: string, key?: string }} [opts]
...
export function registerServer(name, hostname, opts = {}) {
  ...
  return {
    ok: true,
    message: isUpdate
      ? `Updated server "${safeName}" → ${entry.hostname}${entry.user ? ` (user: ${entry.user})` : ''}.`
      : `Registered server "${safeName}" → ${entry.hostname}${entry.user ? ` (user: ${entry.user})` : ''}.`,
  };
}
