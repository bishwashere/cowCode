...
export function onAgentDelegationError({ callerAgentId, targetAgentId, message }) {
  ...
  if (caller) {
    upsertAgent(caller, {
      state: 'error',
      waitingFor: '',
      currentStep: errSummary ? `Delegation failed: ${errSummary}` : 'Delegation failed',
    });
  }
  if (target) {
    upsertAgent(target, {
      state: 'error',
      waitingFor: '',
      currentStep: errSummary ? `Task failed: ${errSummary}` : 'Task failed',
    });
  }
}
...
export function onAgentSkillError({ agentId, skillId, message }) {
  ...
  upsertAgent(id, {
    state: 'error',
    currentStep: errSummary ? `${label}: ${errSummary}` : label,
  });
}
...
export function onAgentTurnError({ agentId, message }) {
  ...
  upsertAgent(id, {
    state: 'error',
    waitingFor: '',
    currentStep: errSummary || 'Turn failed',
  });
}
