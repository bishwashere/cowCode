...
function normalizeTasks(tasks) {
  ...
  return tasks
    .map((t, i) => {
      if (typeof t === 'string') {
        const title = t.trim();
        return title ? { title, status: 'todo' } : null;
      }
...
export function applyProjectSetup(input = {}) {
  if (!input.userApproved) {
    return {
      ok: false,
      needsApproval: true,
      error: 'User approval required. Present the setup preview first, then call apply_setup with userApproved: true.',
    };
  }
...
export function proposeProjectPlan(input = {}) {
  ...
  return {
    ...
    planSteps,
    health,
    note: 'Show this plan to the user and ask for approval. Call apply_plan with userApproved: true to create it.',
  };
}

export function applyProjectPlan(input = {}) {
  if (!input.userApproved) {
    return {
      ok: false,
      needsApproval: true,
      error: 'User approval required. Present the plan first, then call apply_plan with userApproved: true.',
    };
  }
  const project = resolveProjectRef(input.project || input.projectId || input.projectName);
...
export function buildProjectWorkflowContextBlock(opts = {}) {
  ...
  return (
    ...
    '2. **Analyze & propose** — use `project_workflow_propose_plan` with suggested tasks; present the plan clearly to the user.\n' +
    '3. **Approval** — do not create missions or bulk tasks until the user approves. Then call `project_workflow_apply_plan` with `userApproved: true`.\n' +
    '4. **Track progress** — use `project_workflow_log_progress` after meaningful steps; use `project_workflow_update_task` when a task status changes.\n' +
    '5. **Status** — use `project_workflow_status` or `project_workflow_list_projects` when the user asks what is configured.\n' +
