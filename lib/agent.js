...
export async function runAgentTurn({
  ...
  // all parts of the user query were actually addressed. The probe lists ALL missing items
  // at once so the retry can fire parallel tool calls for all of them in a single round.
        ...
        const probeReply = await llmChat([
          ...
          },
          {
            ...
            content:
              ...
              `Did the assistant fully answer ALL parts of the user's question, or did it explicitly say it couldn't answer some parts?\n` +
      ...
      try {
        ...
        if (retryResp.toolCalls?.length) {
          ...
          if (synthesized && !looksLikeBrushOff(synthesized)) {
            finalContent = stripThinking(finalContent).trim() + '\n\n' + synthesized;
          }
        } else {
