...
import { listAgentIds, getAgentMessagingPolicy, toAgentId } from '../agent-config.js';
...
export async function executeAgentSend(ctx, args = {}) {
  ...
  const callerAgentId = toAgentId(ctx?.agentId) || 'main';
  const target = toAgentId(args.agent ?? args.to ?? args.agentId);
  const message = String(args.message ?? args.text ?? '').trim();

  if (!target) return err('agent-send requires "agent" (target agent id).');
  ...
  if (!policy.allow.includes(target)) {
    return err(
      `"${callerAgentId}" is not allowed to message "${target}". ` +
      `Add "${target}" to agentMessaging.allow in this agent's config to permit it.`
    );
  }
