...
import { resolveServer } from '../server-registry.js';

const ALLOWED = new Set(['df', 'du', 'ls', 'pwd', 'cat', 'stat', 'find']);
...
export async function executeSshInspect(ctx, args) {
  const hostArg = (args?.host || '').toString().trim();
  if (!hostArg) {
    return JSON.stringify({ error: 'host is required (e.g. "prod", "db1" or "10.0.0.5").' });
  }
  ...
  // Resolve alias from server registry; fall back to using the value as-is
  const registered = resolveServer(hostArg);
  const hostname = registered?.hostname || hostArg;
  const user = args?.user || registered?.user || process.env.SSH_INSPECT_USER || '';
