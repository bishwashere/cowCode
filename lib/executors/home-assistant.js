 ...
 * Uses HA_URL and HA_TOKEN from environment (loaded from ~/.cowcode/.env when run as daemon).
 */
...
/** Default when HA_URL is not set (Home Assistant default port). */
const DEFAULT_HA_URL = 'http://localhost:8123';

let haEnvLoaded = false;

/** Ensure HA_URL and HA_TOKEN are in process.env; load .env from state dir if missing (daemon case). */
function ensureHaEnv() {
  if (process.env.HA_TOKEN) return; // token is the only required one; URL has a default
...
async function haFetch(path, opts = {}) {
  ...
  try {
    ...
    if (/ECONNREFUSED|connection refused/i.test(msg)) {
      hint = 'Connection refused — is Home Assistant running? If HA is on another machine, set HA_URL in ~/.cowcode/.env (e.g. http://homeassistant.local:8123) and restart the bot.';
    } else if (/ENOTFOUND|getaddrinfo|Unknown host/i.test(msg)) {
      hint = 'Host not found — set the correct HA_URL in ~/.cowcode/.env (e.g. http://homeassistant.local:8123 or http://YOUR_HA_IP:8123) and restart the bot.';
    } else if (/fetch failed|network|timeout|ETIMEDOUT/i.test(msg)) {
      hint = 'Network or timeout error — if HA is not on this machine, set HA_URL in ~/.cowcode/.env to your HA URL and restart the bot.';
    }
