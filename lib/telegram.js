...
export function initBot(token) {
  ...
  bot.on('polling_error', (err) => {
    ...
    if (msg.includes('409') || msg.includes('Conflict') || msg.includes('getUpdates')) {
      ...
      const isTransient = /ETIMEDOUT|ECONNRESET|ENOTFOUND|EAI_AGAIN|socket hang up/i.test(msg);
      const now = Date.now();
      ...
      console.log('[Telegram] Connection issue:', msg.slice(0, 80) + (msg.length > 80 ? '…' : '') + hint);
    }
  });
  return bot;
}
  ...
  return {
    sendMessage: async (chatId, opts) => {
      ...
      if (opts?.image && Buffer.isBuffer(opts.image)) {
        ...
        const sent = await telegramBot.sendPhoto(chatId, opts.image, caption ? { caption } : {});
        return { key: { id: sent.message_id?.toString?.() ?? 'tg-' + Date.now() } };
      }
