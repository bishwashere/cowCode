...
export async function planIntent({ userText, historyMessages = [], availableSkillIds = [], availableSkillSummaries, agentId }) {
  ...
  const userPrompt =
    ...
    `Use the recent conversation when interpreting short or ambiguous messages.\n` +
    `If the user asks you to ask/message another agent, include agent-send when available.\n\n` +
    `Return JSON only:\n` +
    `{\n` +
