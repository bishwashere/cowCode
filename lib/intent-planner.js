...
export async function planIntent({ userText, historyMessages = [], availableSkillIds = [], availableSkillSummaries, agentId, delegationContext = null }) {
  ...
  const delegationSection = (() => {
    ...
    const recommendation = delegationContext.recommendation && typeof delegationContext.recommendation === 'object'
      ...
      : null;
    if (candidates.length === 0 && !recommendation) return '';
    const lines = ['Team delegation context (agent specializations):'];
    ...
    if (recommendation?.targetAgentId) {
      ...
      lines.push(`Recommended delegate: ${recommendation.targetAgentId}${skills}${reason}`);
      lines.push('If this recommendation aligns with the user request, prefer mode="tool" with skills including "agent-send".');
    }
    return lines.join('\n') + '\n\n';
  })();
  const userPrompt =
