...
import { formatHistoryForClassifier, isLikelyContinuationReply } from './conversation-context.js';
 ...
 *
 * @param {{ userText: string, historyMessages?: Array<{role:string,content:string}>, availableSkillIds?: string[], availableSkillSummaries?: Array<{id:string,description:string}>, agentId?: string, continuationHint?: string }} opts
 ...
 */
export async function planIntent({ userText, historyMessages = [], availableSkillIds = [], availableSkillSummaries, agentId, continuationHint }) {
  if (isLikelyContinuationReply(userText, historyMessages, continuationHint)) {
    return {
      mode: 'chat',
      skills: [],
      plan: 'Continue the prior conversation; interpret the short reply in context. No tools.',
      answer_style: 'short',
    };
  }
  ...
  const hintSection = continuationHint ? `Continuation hint: ${continuationHint.slice(0, 300)}\n\n` : '';
  const userPrompt =
    `${historySection}${hintSection}` +
    ...
    `If the latest message is a short reply continuing the recent conversation (picking a choice, confirming, naming), use mode "chat" and skills [].\n\n` +
    `Return JSON only:\n` +
    `{\n` +
